...
但是，相对于它们在平台上的无处不在，以高性能和节省内存的方式来展示它们的行为很容易变成管理不善的努力。 稍微了解UIKit中发生的事情，以及这样处理图像的原因，大家可以获得一些巨大的资源节约，并放弃jetsam的无情愤怒。forgo the unrelenting wrath of jetsam[TODO]
...
突击测试 - 如下我的美丽的女儿的这个 266K ()的照片在 iOS 应用中需要多少内存？
...
Since iOS is efficient at managing power demand when there is a constant need for it and in this case it’ll likely be intermittent断断续续的, it’s good to hop on your own queue to tackle抓住 something like this. This also moves the decoding to the background, which is another significant win.
